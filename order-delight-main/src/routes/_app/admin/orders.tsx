...
export function CustomerOrderActionModule({ order, onActionComplete }: { order: any; onActionComplete?: () => void }) {
  ...
  const rawCount = order?.cancellation_requests_sent ?? order?.cancellationRequestsSent ?? 0;
  const currentRequestsSent = isNaN(parseInt(rawCount, 10)) ? 0 : parseInt(rawCount, 10);
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (updatedOrder: any) => {
      const serverCount = updatedOrder?.cancellation_requests_sent ?? updatedOrder?.cancellationRequestsSent ?? (currentRequestsSent + 1);
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {visibleOrders.map((o: any) => {
            const rawRowCount = o?.cancellation_requests_sent ?? o?.cancellationRequestsSent ?? 0;
            const rowCount = isNaN(parseInt(rawRowCount, 10)) ? 0 : parseInt(rawRowCount, 10);
